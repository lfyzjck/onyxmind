...
import type { CreateSessionResult, Session } from '../../../services/session-manager';
...
export function useChatController(plugin: OnyxMindPlugin): UseChatControllerResult {
	...
	const updateSlashState = useCallback((text: string, cursor: number): void => {
	...
	const handleSessionCreationError = useCallback((result: CreateSessionResult) => {
	...
	const applySlashCommand = useCallback((command: AvailableCommand): void => {
		if (slashStart === null) {
			return;
		}

		const replaceEnd = slashEnd ?? inputRef.current?.selectionStart ?? inputText.length;
				...
				text,
				{
					...
					onToolUse: (chunk) => {
						setStreamTools((prev) => {
							...
							return {
								...prev,
								[chunk.partId]: prevChunk ? { ...prevChunk, ...chunk } : chunk,
							};
						});
					},
					onError: (error) => {
	...
	}, [appendError, handleSessionCreationError, plugin, refreshSessionState, resetStreamingState]);
	...
	const handleInputClick = useCallback((value: string, cursor: number) => {
		updateSlashState(value, cursor);
	}, [updateSlashState]);

	const handleInputKeyUp = useCallback((value: string, cursor: number, key: string) => {
	...
	const handleInputKeyDown = useCallback((event: KeyboardEvent<HTMLTextAreaElement>) => {
		if (slashMenuOpen) {
			...
			if ((event.key === 'Enter' || event.key === 'Tab') && !event.shiftKey && !isComposing) {
				if (filteredCommands.length > 0) {
					event.preventDefault();
	...
	}, [activeSessionId, activeMessages.length, streamText, streamThinking, toolChunks.length, errors.length, isStreaming]);
	...
	return {
		...
		handleSwitchSession,
		handleCloseSession: (sessionId: string) => void handleCloseSession(sessionId),
